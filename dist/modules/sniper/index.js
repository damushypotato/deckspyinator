...
export async function snipe(key, playerName, clanName, output = false) {
    let clanSearch;
    const maxAttempts = 3;
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        try {
            ...
            if (clanSearch && clanSearch.length > 0) {
                ...
                if (output)
                    console.log(`Attempt ${attempt}: No clans found.`);
            }
        }
        catch (error) {
            if (output)
                console.error(`Attempt ${attempt}: Error occurred - ${error.message}`);
        }
        await new Promise(resolve => setTimeout(resolve, 50));
    }
    if (!clanSearch || clanSearch.length === 0) {
        if (output)
            console.log('Unable to find clan after maximum attempts.');
        return;
    }
    ...
    if (output) {
        console.log(`Best player match found: ${bestMatch.name} (${bestMatch.tag}), Similarity: ${bestMatch.similarity}`);
    }
    ...
    if (output) {
        ...
        playerInfo.currentDeck.forEach(card => {
            ...
            console.log(`${card.name}: Level ${trueLevel}${card.evolutionLevel ? `, Evolution Level ${card.evolutionLevel}` : ''}`);
        });
    }
    return {
        player: playerInfo,
